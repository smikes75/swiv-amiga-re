...
Zbyvajici rozdily zahrnuji RNG, faze animaci a casovani zachytu. Textura
terenu je soucast originalnich dlazdic; gamma profil se prevadi jednou.

    python3 tools/compare.py            # ctyri simulovane TOWN checkpointy
    python3 tools/compare.py start      # jeden checkpoint
    python3 tools/compare.py desert     # samostatna staticka kontrola terenu
"""
...
CHECKPOINTS = {
    # Preserved local renderer checkpoint. Its original frame includes
    # actors, the remake deliberately isolates terrain/HUD/HELI. Keep the
    # original 24/channel tolerance and 95.5% floor; this is not a gameplay
    # or timing oracle. A fresh capture emulates 310 seconds.
    "desert": {"t": 310, "row": 5426, "floor": 95.5, "level": 1,
               "static": True, "ticks": 0, "tolerance": 24},
...
DEFAULT_CHECKPOINTS = ("start", "wave", "death", "respawn")
    ...
    with sync_playwright() as pw:
        ...
        try:
            ...
            encoded = page.evaluate("""(cp) => {
              ...
              if (cp.static) {
                g.scroll = (g.rowOffset | 0) + cp.row;
                g.fadeBlack = 0; g.fadeDir = 0; g.fadeWhite = 0;
              ...
              const hudTexts = hudTextsForGame(g);
              const hudKey = hudTexts.left + '\\0' + hudTexts.right;
...
def match_percent(a, b, pixels=None, tolerance=TOLERANCE):
    if pixels is None:
        width, height = FRAME_SIZE
...
def main():
    ...
    for name in names:
        ...
        tolerance = checkpoint.get("tolerance", TOLERANCE)
        scores = {
            "whole": match_percent(original, frames["whole"], masks["whole"], tolerance),
            "terrain": match_percent(original, frames["terrain"],
                                     masks["terrain"], tolerance),
            "HUD": match_percent(original, frames["whole"], masks["HUD"], tolerance),
            "HELI": match_percent(original, frames["whole"], masks["HELI"], tolerance),
        }
        ...
        print(f"{name}: t={checkpoint['t']}, row={checkpoint['row']}, "
              f"ticks={checkpoint['ticks']}, tolerance={tolerance}, "
              f"mode={'static renderer' if checkpoint.get('static') else 'simulation'}")
